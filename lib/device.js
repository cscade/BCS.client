...
var Device = function (host, port, next) {
	...
	this.read('system.model', function (e, device) {
		...
		if (device === 'BCS-460' || device === 'BCS-462') {
			...
			if (device === 'BCS-460') that.dictionary = require('./dictionary-460');
			if (device === 'BCS-462') that.dictionary = require('./dictionary-462');
...
Device.prototype.lookup = function (path) {
	...
	if (path === 'system.model' && this.info.type === undefined) return require('./dictionary-460').system.model;
	if (!this.dictionary) return;
	path = path.split('.');
	section = path[0];
	key = path[1];
	if (this.dictionary[section] && this.dictionary[section][key]) return this.dictionary[section][key];
	return;
};
