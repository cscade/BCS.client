...
Cache.prototype.get = function (endpoint, next) {
	...
	if (this.running) return setImmediate(function () { that.get(endpoint, next); });
	...
	this.running = true;
	request({
		url: 'http://' + this.location.host + ':' + (this.location.port || 80) + '/api/' + endpoint,
...
Cache.prototype.cacheable = function (endpoint) {
	var root = endpoint.split('/')[0];

	// Determine if an endpoint is cacheable
	return this.endpoints[root] === 'static';
};
